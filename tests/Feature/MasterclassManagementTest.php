...
class MasterclassManagementTest extends TestCase
{
    ...
    #[Test]
    public function leader_cannot_create_masterclass_with_past_date(): void
    {
        $leader = User::factory()->leader()->create();
        $category = CraftCategory::factory()->create();
        ...
        $this->actingAs($leader)
            ...
            ->post(route('masterclass.store'), [
                'category_id' => $category->id,
                'title' => 'Прошлый мастер-класс',
                'description' => 'Описание',
                'mc_date' => now()->subDay()->format('Y-m-d'),
                'start_time' => '11:00:00',
                'max_participants' => 8,
                'price' => 1500,
            ])
            ->assertRedirect(route('masterclass.create'))
            ->assertSessionHasErrors('mc_date');

        $this->assertDatabaseMissing('masterclasses', [
            'leader_id' => $leader->id,
            'title' => 'Прошлый мастер-класс',
        ]);
    }
